/* Board tests for Problems 1–3 (parse/render, rules, concurrency)
   Matches API:
   - Board.parseFromFile(filename)
   - board.snapshot(player)
   - board.flipFirst({r,c}, player) / board.flipSecond({r,c}, player)
   - board.flipFirstAsync({r,c}, player) / board.flipSecondAsync({r,c}, player)
*/
